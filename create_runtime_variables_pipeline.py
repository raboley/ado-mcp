...
async def create_runtime_variables_pipeline():
    ...
    async with Client(mcp) as client:
        ...
        print(f"Found GitHub service connection: {github_connection['name']} (ID: {github_connection['id']})")
        ...
        try:
            pipeline = await client.call_tool("create_pipeline", {
            ...
            print(f"  Pipeline URL: https://dev.azure.com/RussellBoley/ado-mcp/_build?definitionId={pipeline.data.id}")
            ...
            if "409" in str(e):
                ...
                for p in pipelines.data:
                    if p["name"] == pipeline_name:
                        ...
                        print(f"Pipeline URL: https://dev.azure.com/RussellBoley/ado-mcp/_build?definitionId={p['id']}")
                        return p

...
class TestConnectionPooling:
    ...
    def test_client_uses_requests_module_when_pooling_disabled(
        self, non_pooled_config, mock_auth_manager
    ):
        """Test that client uses requests module when connection pooling is disabled."""
        client = AdoClient(
            organization_url="https://dev.azure.com/test", pat="test-pat", config=non_pooled_config
        )
    ...
    def test_session_configured_with_pool_settings(self, pooled_config, mock_auth_manager):
        ...
        with (
            patch("ado.client.requests.Session") as mock_session_class,
            ...
            mock_adapter_class.assert_called_once_with(
                pool_connections=10, pool_maxsize=50, pool_block=False
            )
    ...
    def test_send_request_uses_session_when_available(self, pooled_config, mock_auth_manager):
        ...
        with patch("ado.client.requests.Session") as mock_session_class:
            ...
            # Make a request
            result = client._send_request(method="GET", url="https://dev.azure.com/test/_apis/test")

            # Verify session.request was called, not requests.request
            mock_session.request.assert_called_once()
            assert result == {"test": "data"}

    def test_work_items_client_uses_session_for_create_operations(
        self, pooled_config, mock_auth_manager
    ):
        ...
        with patch("ado.client.requests.Session") as mock_session_class:
            ...
            result = work_items_client.create_work_item(
                project_id="test-project", work_item_type="Bug", fields={"System.Title": "Test Bug"}
            )
    ...
    def test_work_items_client_uses_session_for_update_operations(
        self, pooled_config, mock_auth_manager
    ):
    ...
    def test_connection_pool_config_validation(self):
        ...
        valid_config = ConnectionPoolConfig(
            enabled=True, max_pool_connections=10, max_pool_size=50, block=False, pool_timeout=5.0
        )

...
async def test_get_my_work_items_basic_functionality(client, project_id):
    ...
    try:
        ...
        result = await client.call_tool(
            "get_my_work_items", {"project_id": project_id, "assigned_to": current_user}
        )

        assert result.is_error is False, (
            f"Should get my work items successfully but got error: {result.content}"
        )
        assert "work_items" in result.data, (
            f"Result should contain work_items but got: {result.data.keys()}"
        )
        assert "pagination" in result.data, (
            f"Result should contain pagination but got: {result.data.keys()}"
        )
        assert "assignment_info" in result.data, (
            f"Result should contain assignment_info but got: {result.data.keys()}"
        )

        # Verify assignment info
        assignment_info = result.data["assignment_info"]
        assert assignment_info["assigned_to"] == current_user, (
            f"Should filter by current user but got: {assignment_info['assigned_to']}"
        )

        # Find our created work item
        work_item_ids = [item["id"] for item in result.data["work_items"]]
        assert work_item_id in work_item_ids, (
            f"Should find our created work item {work_item_id} in results: {work_item_ids}"
        )
...
async def test_get_my_work_items_with_filters(client, project_id):
    ...
    bug_result = await client.call_tool(
        "create_work_item",
        {
            "project_id": project_id,
            "work_item_type": "Bug",
    ...
    assert bug_result.is_error is False, (
        f"Should create bug successfully but got error: {bug_result.content}"
    )
    assert task_result.is_error is False, (
        f"Should create task successfully but got error: {task_result.content}"
    )
    ...
    await client.call_tool(
        "update_work_item", {"project_id": project_id, "work_item_id": task_id, "state": "Active"}
    )

    try:
        # Test filtering by work item type
        bugs_result = await client.call_tool(
            "get_my_work_items",
            {"project_id": project_id, "assigned_to": current_user, "work_item_type": "Bug"},
        )

        assert bugs_result.is_error is False, (
            f"Should get bugs successfully but got error: {bugs_result.content}"
        )
        ...
        assert assignment_info["type_filter"] == "Bug", (
            f"Should show Bug filter but got: {assignment_info['type_filter']}"
        )

        # Test filtering by state
        new_items_result = await client.call_tool(
            "get_my_work_items",
            {"project_id": project_id, "assigned_to": current_user, "state": "New"},
        )

        assert new_items_result.is_error is False, (
            f"Should get new items successfully but got error: {new_items_result.content}"
        )
        ...
        assert assignment_info["state_filter"] == "New", (
            f"Should show New state filter but got: {assignment_info['state_filter']}"
        )

    finally:
        # Clean up
        await client.call_tool(
            "delete_work_item", {"project_id": project_id, "work_item_id": bug_id}
        )
        await client.call_tool(
...
async def test_get_recent_work_items_basic_functionality(client, project_id):
    ...
    try:
        ...
        result = await client.call_tool("get_recent_work_items", {"project_id": project_id})

        assert result.is_error is False, (
            f"Should get recent work items successfully but got error: {result.content}"
        )
        assert "work_items" in result.data, (
            f"Result should contain work_items but got: {result.data.keys()}"
        )
        ...
        assert "time_filter" in result.data, (
            f"Result should contain time_filter but got: {result.data.keys()}"
        )
        ...
        work_item_ids = [item["id"] for item in result.data["work_items"]]
        assert work_item_id in work_item_ids, (
            f"Should find our recently created work item {work_item_id} in results: {work_item_ids}"
        )
...
async def test_get_recent_work_items_with_custom_days(client, project_id):
    ...
    try:
        ...
        result = await client.call_tool(
            "get_recent_work_items", {"project_id": project_id, "days": 1, "work_item_type": "Bug"}
        )
        ...
        assert time_filter["type_filter"] == "Bug", (
            f"Should filter by Bug type but got: {time_filter['type_filter']}"
        )

        # Should find our recently created bug
        work_item_ids = [item["id"] for item in result.data["work_items"]]
        assert work_item_id in work_item_ids, (
            f"Should find our recently created bug {work_item_id} in results: {work_item_ids}"
        )
...
async def test_get_recent_work_items_with_state_filter(client, project_id):
    ...
    new_item_result = await client.call_tool(
        "create_work_item",
        {
            "project_id": project_id,
            "work_item_type": "Task",
    ...
    assert new_item_result.is_error is False, (
        f"Should create new task successfully but got error: {new_item_result.content}"
    )
    ...
    try:
        ...
        result = await client.call_tool(
            "get_recent_work_items", {"project_id": project_id, "state": "New", "days": 1}
        )

        assert result.is_error is False, (
            f"Should get recent new items successfully but got error: {result.content}"
        )

        # Verify state filter
        time_filter = result.data["time_filter"]
        assert time_filter["state_filter"] == "New", (
            f"Should filter by New state but got: {time_filter['state_filter']}"
        )

        # Should find our new item
        work_item_ids = [item["id"] for item in result.data["work_items"]]
        assert new_item_id in work_item_ids, (
            f"Should find our new item {new_item_id} in results: {work_item_ids}"
        )
...
async def test_get_recent_work_items_pagination(client, project_id):
    ...
    result = await client.call_tool(
        "get_recent_work_items", {"project_id": project_id, "page_size": 2, "page_number": 1}
    )

    assert result.is_error is False, (
        f"Should get recent work items with pagination successfully but got error: {result.content}"
    )

    # Verify pagination metadata
    pagination = result.data["pagination"]
    assert pagination["page_number"] == 1, (
        f"Should be on page 1 but got: {pagination['page_number']}"
    )
    assert pagination["page_size"] == 2, (
        f"Should have page size 2 but got: {pagination['page_size']}"
    )
    assert pagination["items_count"] <= 2, (
        f"Should have at most 2 items but got: {pagination['items_count']}"
    )
...
async def test_convenience_tools_error_handling(client):
    ...
    assert "400" in str(exc_info.value) or "not found" in str(exc_info.value).lower(), (
        f"Should get bad request error but got: {exc_info.value}"
    )

    # Test get_recent_work_items with invalid project
    with pytest.raises(Exception) as exc_info:
        await client.call_tool(
            "get_recent_work_items", {"project_id": "00000000-0000-0000-0000-000000000000"}
        )

...
class TestProjectManagementWorkflow:
    ...
    async def test_simple_workflow_with_tasks_and_bugs(self, mcp_client, project_id, workflow_cleanup):
        """Test simple workflow with only Task and Bug work item types."""
        
        # Step 1: Create main Task (using minimal required fields)
        main_task_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Task",
        ...
        assert main_task_result.data["fields"]["System.Title"] == "Integration Test: Implement Customer Management Feature"
        ...
        for task_title in task_templates:
            sub_task_result = await mcp_client.call_tool("create_work_item", {
                "project_id": project_id,
                "work_item_type": "Task",
        ...
        main_task_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": main_task_id
        })
        
        # Main task should have relationships to sub-tasks
        assert len(main_task_relations.data) >= len(sub_tasks), f"Main task should have at least {len(sub_tasks)} relationships but got {len(main_task_relations.data)}"
        
        # Step 4: Verify a sub-task has relationship to main task
        sub_task_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": sub_tasks[0]["id"]
        })
        
        # Sub-task should have relationship to main task
        assert len(sub_task_relations.data) >= 1, f"Sub-task should have at least 1 relationship but got {len(sub_task_relations.data)}"
        ...
        all_items_query = await mcp_client.call_tool("query_work_items", {
            "project_id": project_id,
            "simple_filter": {"tags": "integration-test"}
        })
        
        # Should find at least the main task (which now has the tag)
        assert len(all_items_query.data["workItems"]) >= 1, "Should find work items with integration-test tag"
    ...
    async def test_bug_lifecycle_workflow(self, mcp_client, project_id, workflow_cleanup):
        ...
        bug_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Bug",
        ...
        bug_update = await mcp_client.call_tool("update_work_item", {
            "project_id": project_id,
            "work_item_id": bug_id,
        ...
        fix_task_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Task",
        ...
        test_case_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
        ...
        bug_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": bug_id
        })
        
        # Bug should have relationships to both Task and Test Case
        assert len(bug_relations.data) >= 2, f"Bug should have at least 2 relationships but got {len(bug_relations.data)}"
        
        # Verify comments were added
        bug_comments = await mcp_client.call_tool("get_work_item_comments", {
            "project_id": project_id,
            "work_item_id": bug_id
        })
        
        assert len(bug_comments.data) >= 3, f"Bug should have at least 3 comments but got {len(bug_comments.data)}"
        
        # Verify work item history
        bug_history = await mcp_client.call_tool("get_work_item_history", {
            "project_id": project_id,
            "work_item_id": bug_id
        })
        
        assert len(bug_history.data) >= 2, f"Bug should have multiple history entries but got {len(bug_history.data)}"
    ...
    async def test_dependency_management_workflow(self, mcp_client, project_id, workflow_cleanup):
        ...
        database_task = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Task", 
            "title": "Setup database schema"
        })
        
        api_task = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Task",
            "title": "Implement REST API endpoints"
        })
        
        ui_task = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Task",
        ...
        db_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": db_id
        })
        ...
        api_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": api_id
        })
        ...
        assert len(api_dependency_relations) >= 2, "API should have dependency relationships both ways"
        
        # Step 5: Query work items by tag to verify all are created
        tagged_items = await mcp_client.call_tool("query_work_items", {
            "project_id": project_id,
            "simple_filter": {"tags": "integration-test"}
        })
    ...
    async def test_batch_operations_workflow(self, mcp_client, project_id, workflow_cleanup):
        ...
        batch_result = await mcp_client.call_tool("get_work_items_batch", {
            "project_id": project_id,
            "work_item_ids": work_items,
            "fields": ["System.Id", "System.Title", "System.State", "System.WorkItemType"]
        })
        
        assert len(batch_result.data) == len(work_items), f"Should retrieve all {len(work_items)} work items in batch"
        ...
        for work_item_id in work_items:
            update_operations.append({
                "work_item_id": work_item_id,
                "operations": [
                    {"op": "replace", "path": "/fields/System.Description", "value": "Updated via batch operation"},
                    {"op": "replace", "path": "/fields/System.Tags", "value": "integration-test; batch"}
                ]
            })
        
        batch_update_result = await mcp_client.call_tool("update_work_items_batch", {
            "project_id": project_id,
            "work_item_updates": update_operations
        })
        
        assert len(batch_update_result.data) == len(work_items), f"Should update all {len(work_items)} work items in batch"
        
        # Step 4: Verify all items were updated
        updated_batch = await mcp_client.call_tool("get_work_items_batch", {
            "project_id": project_id,
            "work_item_ids": work_items,
        ...
        for item in updated_batch.data:
            assert item["fields"].get("System.Description") == "Updated via batch operation", f"Work item {item['id']} should have updated description"
            assert "batch" in item["fields"].get("System.Tags", ""), f"Work item {item['id']} should have batch tag"
        
        # Step 5: Query to find all batch test items
        batch_query = await mcp_client.call_tool("query_work_items", {
            "project_id": project_id,
            "simple_filter": {
                "tags": "batch"
            }
        ...
        assert len(batch_query.data["workItems"]) >= len(work_items), "Should find all batch test items with batch tag"
...
class TestDataIntegrityWorkflow:
    ...
    async def test_relationship_data_integrity(self, mcp_client, project_id, workflow_cleanup):
        ...
        parent_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
            "work_item_type": "Epic",
            "title": "Data Integrity Test Epic"
        })
        
        child_result = await mcp_client.call_tool("create_work_item", {
            "project_id": project_id,
        ...
        parent_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": parent_id
        })
        
        child_relations = await mcp_client.call_tool("get_work_item_relations", {
            "project_id": project_id,
            "work_item_id": child_id
        })
        
        # Parent should have forward relationship
        forward_relations = [rel for rel in parent_relations.data if rel["rel"] == "System.LinkTypes.Hierarchy-Forward"]
        assert len(forward_relations) >= 1, "Parent should have hierarchy-forward relationship"
        
        # Child should have reverse relationship  
        reverse_relations = [rel for rel in child_relations.data if rel["rel"] == "System.LinkTypes.Hierarchy-Reverse"]
    ...
    async def test_concurrent_operations_workflow(self, mcp_client, project_id, workflow_cleanup):
        ...
        # Verify final state
        final_result = await mcp_client.call_tool("get_work_item", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        
        # Should have the latest values
        assert final_result.data["fields"]["System.Description"] == "Updated description for concurrent test"

...
async def test_update_work_items_batch_basic_functionality(client, project_id):
    ...
    try:
        ...
        result = await client.call_tool("update_work_items_batch", {
            "project_id": project_id,
            "work_item_updates": work_item_updates
        })
        
        assert result.is_error is False, f"Should update work items batch successfully but got error: {result.content}"
        ...
        assert isinstance(updated_items, list), f"Result should be a list but got: {type(updated_items)}"
        assert len(updated_items) == 3, f"Should return 3 updated work items but got: {len(updated_items)}"
        
        # Verify all work items were updated correctly
        returned_ids = [item["id"] for item in updated_items]
        for work_item_id in work_item_ids:
            assert work_item_id in returned_ids, f"Updated work item {work_item_id} should be in returned items: {returned_ids}"
        ...
        for i in range(3):
            ...
            assert expected_title in updated_titles, f"Expected title '{expected_title}' should be in updated titles: {updated_titles}"
...
async def test_update_work_items_batch_error_handling_fail_policy(client, project_id):
    ...
    try:
        ...
        with pytest.raises(Exception) as exc_info:
            await client.call_tool("update_work_items_batch", {
                "project_id": project_id,
                "work_item_updates": work_item_updates,
                "error_policy": "fail"
            })
...
async def test_update_work_items_batch_error_handling_omit_policy(client, project_id):
    ...
    try:
        ...
        work_item_updates = [
            {
                "work_item_id": valid_id,
                "operations": [
                    {"op": "replace", "path": "/fields/System.Title", "value": "Updated valid item via omit policy"}
                ]
            },
            {
                "work_item_id": invalid_id,
                "operations": [
                    {"op": "replace", "path": "/fields/System.Title", "value": "This should be omitted"}
                ]
            }
        ]
        ...
        assert len(result.data) <= 2, f"Should return at most 2 work items but got: {len(result.data)}"
        assert len(result.data) >= 1, f"Should return at least 1 valid work item but got: {len(result.data)}"
        ...
        for item in updated_items:
            if item["id"] == valid_id:
                found_valid = True
                assert item["fields"]["System.Title"] == "Updated valid item via omit policy", f"Valid item should be updated"
                break
...
async def test_update_work_items_batch_too_many_items(client, project_id):
    ...
    with pytest.raises(Exception) as exc_info:
        await client.call_tool("update_work_items_batch", {
            "project_id": project_id,
            "work_item_updates": large_update_list
        })
...
async def test_update_work_items_batch_validation_only(client, project_id):
    ...
    try:
        ...
        work_item_updates = [
            {
                "work_item_id": work_item_id,
                "operations": [
                    {"op": "replace", "path": "/fields/System.Title", "value": "Validation test title"}
                ]
            }
        ]
        ...
        assert result.is_error is False, f"Should validate successfully but got error: {result.content}"
        ...
        assert get_result.data["fields"]["System.Title"] == original_title, f"Work item should not be updated during validation-only operation"
...
async def test_update_work_items_batch_malformed_operations(client, project_id):
    ...
    try:
        ...
        assert "work_item_id" in str(exc_info.value), f"Should complain about missing work_item_id: {exc_info.value}"
        ...
        assert "operations" in str(exc_info.value), f"Should complain about missing operations: {exc_info.value}"

...
async def test_work_item_types_caching(mcp_client, project_id):
    ...
    result1 = await mcp_client.call_tool("list_work_item_types", {
    ...
    assert isinstance(work_item_types1, list), f"Should return a list but got: {type(work_item_types1)}"
    assert len(work_item_types1) > 0, "Should have at least one work item type"
    
    # Second call - should hit the cache
    start_time = time.time()
    result2 = await mcp_client.call_tool("list_work_item_types", {
        "project_id": project_id
    })
    ...
    assert len(work_item_types1) == len(work_item_types2), "Should return same number of work item types from cache"
    assert work_item_types1[0]["name"] == work_item_types2[0]["name"], "Should return same work item types from cache"
    
    # Cache should be significantly faster (at least 5x faster)
    assert second_call_duration < first_call_duration / 5, f"Cache should be faster. API: {first_call_duration:.3f}s, Cache: {second_call_duration:.3f}s"
...
async def test_area_paths_caching(mcp_client, project_id):
    ...
    if area_paths1:  # Only check if we have data
        assert area_paths1[0]["name"] == area_paths2[0]["name"], "Should return same area path data from cache"
...
async def test_iteration_paths_caching(mcp_client, project_id):
    ...
    # First call - should hit the API
    start_time = time.time()
    result1 = await mcp_client.call_tool("list_iteration_paths", {
        "project_id": project_id
    })
    first_call_duration = time.time() - start_time
    ...
    assert isinstance(iteration_paths1, list), f"Should return a list but got: {type(iteration_paths1)}"
    
    # Second call - should hit the cache
    start_time = time.time()
    result2 = await mcp_client.call_tool("list_iteration_paths", {
        "project_id": project_id
    })
    second_call_duration = time.time() - start_time
    ...
    # Verify same data returned
    assert len(iteration_paths1) == len(iteration_paths2), "Should return same iteration paths from cache"
    if iteration_paths1:  # Only check if we have data
        assert iteration_paths1[0]["name"] == iteration_paths2[0]["name"], "Should return same iteration path data from cache"
...
async def test_cache_expiration_not_immediate(mcp_client, project_id):
    ...
    assert cache_call_duration < 0.1, f"Cache call should be fast but took {cache_call_duration:.3f}s"
...
async def test_cache_different_projects_isolated(mcp_client, project_id):
    ...
    try:
        await mcp_client.call_tool("list_work_item_types", {
...
async def test_work_item_types_cache_integration(mcp_client, project_id):
    ...
    assert stats_after_first["total_entries"] >= 2, "Should have cached work item types and name map"
    assert stats_after_first["active_entries"] == stats_after_first["total_entries"], "All entries should be active"
    
    # Verify cache keys are as expected
    cache_keys = stats_after_first["cache_keys"]
    assert any(f"work_item_types:{project_id}" in key for key in cache_keys), "Should have work item types cache key"
    assert any(f"work_item_types:{project_id}:name_map" in key for key in cache_keys), "Should have name map cache key"
...
async def test_list_work_item_types_returns_valid_data(mcp_client, project_id):
    result = await mcp_client.call_tool("list_work_item_types", {"project_id": project_id})
    ...
    assert isinstance(work_item_types, list), (
        f"Work item types should be a list but got: {type(work_item_types)}"
    )
    assert len(work_item_types) > 0, (
        f"Should have at least one work item type but got: {work_item_types}"
    )

    # Check structure of first work item type
    first_type = work_item_types[0]
    assert isinstance(first_type, dict), (
        f"Work item type should be a dictionary but got: {type(first_type)}"
    )
    assert "name" in first_type, (
        f"Work item type should have 'name' field but got keys: {list(first_type.keys())}"
    )
    assert "referenceName" in first_type, (
        f"Work item type should have 'referenceName' field but got keys: {list(first_type.keys())}"
    )
...
async def test_get_work_item_type_fields_for_bug(mcp_client, project_id):
    result = await mcp_client.call_tool(
        "get_work_item_type_fields", {"project_id": project_id, "work_item_type": "Bug"}
    )
    ...
    assert isinstance(first_field, dict), (
        f"Field should be a dictionary but got: {type(first_field)}"
    )
    assert "referenceName" in first_field, (
        f"Field should have 'referenceName' but got keys: {list(first_field.keys())}"
    )
    assert "name" in first_field, (
        f"Field should have 'name' but got keys: {list(first_field.keys())}"
    )
    assert "type" in first_field, (
        f"Field should have 'type' but got keys: {list(first_field.keys())}"
    )
...
async def test_get_work_item_type_fields_for_task(mcp_client, project_id):
    result = await mcp_client.call_tool(
        "get_work_item_type_fields", {"project_id": project_id, "work_item_type": "Task"}
    )
...
async def test_get_work_item_type_fields_invalid_type(mcp_client, project_id):
    with pytest.raises(Exception) as exc_info:
        await mcp_client.call_tool(
            ...
            {"project_id": project_id, "work_item_type": "InvalidWorkItemType"},
        )
...
async def test_list_area_paths_returns_valid_data(mcp_client, project_id):
    result = await mcp_client.call_tool("list_area_paths", {"project_id": project_id})
    ...
    assert isinstance(root_node, dict), (
        f"Area path node should be a dictionary but got: {type(root_node)}"
    )
    assert "name" in root_node, (
        f"Area path should have 'name' field but got keys: {list(root_node.keys())}"
    )
...
async def test_list_area_paths_with_depth_limit(mcp_client, project_id):
    result = await mcp_client.call_tool("list_area_paths", {"project_id": project_id, "depth": 1})
...
async def test_list_iteration_paths_returns_valid_data(mcp_client, project_id):
    result = await mcp_client.call_tool("list_iteration_paths", {"project_id": project_id})

    assert result.data is not None, "Iteration paths should return data"
    iteration_paths = result.data
    assert isinstance(iteration_paths, list), (
        f"Iteration paths should be a list but got: {type(iteration_paths)}"
    )
    assert len(iteration_paths) > 0, (
        f"Should have at least one iteration path but got: {iteration_paths}"
    )

    # Check structure of iteration path node
    root_node = iteration_paths[0]
    assert isinstance(root_node, dict), (
        f"Iteration path node should be a dictionary but got: {type(root_node)}"
    )
    assert "name" in root_node, (
        f"Iteration path should have 'name' field but got keys: {list(root_node.keys())}"
    )
    assert "path" in root_node, (
...
async def test_list_iteration_paths_with_depth_limit(mcp_client, project_id):
    result = await mcp_client.call_tool(
        "list_iteration_paths", {"project_id": project_id, "depth": 2}
    )
...
async def test_work_item_type_field_validation_properties(mcp_client, project_id):
    ...
    title_field = next(
        (field for field in fields if field["referenceName"] == "System.Title"), None
    )
    assert title_field is not None, (
        f"Should find System.Title field but available fields are: {[f['referenceName'] for f in fields]}"
    )

    # Title should be required
    is_required = title_field.get("alwaysRequired", False) or title_field.get("required", False)
    assert is_required is True, (
        f"System.Title should be required but got alwaysRequired: {title_field.get('alwaysRequired')}, required: {title_field.get('required')}"
    )

    # Find the System.State field
    state_field = next(
        (field for field in fields if field["referenceName"] == "System.State"), None
    )
    assert state_field is not None, (
        f"Should find System.State field but available fields are: {[f['referenceName'] for f in fields]}"
    )
    ...
    if allowed_values:
        assert isinstance(allowed_values, list), (
            f"State allowedValues should be a list but got: {type(allowed_values)}"
        )
        assert len(allowed_values) > 0, (
            f"State should have allowed values but got: {allowed_values}"
        )
        # Common Bug states
        ...
        assert len(found_states) >= 2, (
            f"Should find at least 2 common states but found: {found_states} in {allowed_values}"
        )

...
async def test_get_work_items_batch_basic_functionality(client, project_id):
    ...
    try:
        ...
        result = await client.call_tool("get_work_items_batch", {
            "project_id": project_id,
            "work_item_ids": work_item_ids
        })
        
        assert result.is_error is False, f"Should get work items batch successfully but got error: {result.content}"
        assert isinstance(result.data, list), f"Result should be a list but got: {type(result.data)}"
        ...
        for expected_id in work_item_ids:
            assert expected_id in returned_ids, f"Work item {expected_id} should be in returned items"
...
async def test_get_work_items_batch_with_field_filtering(client, project_id):
    ...
    try:
        ...
        assert result.is_error is False, f"Should get work items with field filtering but got error: {result.content}"
...
async def test_get_work_items_batch_error_handling_fail_policy(client, project_id):
    ...
    try:
        ...
        try:
            ...
            assert "404" in str(exc) or "not found" in str(exc).lower(), f"Should get not found error but got: {exc}"
...
async def test_get_work_items_batch_too_many_ids(client, project_id):
    ...
    with pytest.raises(Exception) as exc_info:
        await client.call_tool("get_work_items_batch", {
            "project_id": project_id,

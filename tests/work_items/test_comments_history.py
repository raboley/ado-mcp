...
class TestWorkItemComments:
    ...
    async def test_add_work_item_comment_basic_functionality(self, mcp_client, project_id, sample_work_item_id):
        """Test basic functionality of adding a work item comment."""
        comment_text = "This is a test comment from automated testing"
        
        result = await mcp_client.call_tool("add_work_item_comment", {
            "project_id": project_id,
        ...
        assert result.data["text"] == comment_text, f"Comment text should match input: expected '{comment_text}', got '{result.data['text']}'"
        assert result.data["work_item_id"] == sample_work_item_id, f"Comment should be associated with work item {sample_work_item_id}"
        assert result.data["format"] == "html", f"Comment format should be 'html', got '{result.data['format']}'"
    ...
    async def test_add_work_item_comment_with_markdown(self, mcp_client, project_id, sample_work_item_id):
        ...
        assert result.data is not None, f"Should successfully add markdown comment but got: {result}"
        assert result.data["text"] == markdown_text, f"Markdown text should be preserved: expected '{markdown_text}', got '{result.data['text']}'"
        # Note: Azure DevOps may convert markdown format to HTML in the response
        assert result.data["format"] in ["markdown", "html"], f"Comment format should be 'markdown' or 'html', got '{result.data['format']}'"
    ...
    async def test_add_work_item_comment_invalid_work_item(self, mcp_client, project_id):
        ...
        with pytest.raises(Exception) as exc_info:
            await mcp_client.call_tool("add_work_item_comment", {
                "project_id": project_id,
...
class TestWorkItemCommentRetrieval:
    ...
    async def test_get_work_item_comments_basic_functionality(self, mcp_client, project_id, work_item_with_comments):
        ...
        result = await mcp_client.call_tool("get_work_item_comments", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        
        assert result.data is not None, f"Should successfully retrieve comments but got: {result}"
        assert len(result.data) >= expected_comment_count, f"Should retrieve at least {expected_comment_count} comments but got {len(result.data)}"
        ...
        for comment in result.data:
            ...
            assert comment["work_item_id"] == work_item_id, f"Comment should belong to work item {work_item_id}"

    async def test_get_work_item_comments_pagination(self, mcp_client, project_id, work_item_with_comments):
        ...
        assert result.data is not None, f"Should successfully retrieve paginated comments but got: {result}"
    ...
    async def test_get_work_item_comments_empty_work_item(self, mcp_client, project_id):
        ...
        assert result.data is not None, f"Should successfully retrieve empty comment list but got: {result}"
        assert len(result.data) == 0, f"Should return empty list for work item with no comments but got {len(result.data)} comments"
...
class TestWorkItemHistory:
    ...
    async def work_item_with_history(self, mcp_client, project_id, test_work_item):
        ...
        for update in updates:
            await mcp_client.call_tool("update_work_item", {
                "project_id": project_id,
                "work_item_id": work_item_id,
    ...
    async def test_get_work_item_history_basic_functionality(self, mcp_client, project_id, work_item_with_history):
        """Test basic functionality of retrieving work item history."""
        work_item_id = work_item_with_history
        
        result = await mcp_client.call_tool("get_work_item_history", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        
        assert result.data is not None, f"Should successfully retrieve history but got: {result}"
        assert len(result.data) > 0, f"Should have at least one revision in history but got {len(result.data)}"
        ...
        for revision in result.data:
            ...
            assert revision["id"] == work_item_id, f"Revision should belong to work item {work_item_id}"
            assert isinstance(revision["rev"], int), f"Revision number should be an integer, got {type(revision['rev'])}"
            assert isinstance(revision["fields"], dict), f"Fields should be a dictionary, got {type(revision['fields'])}"

    async def test_get_work_item_history_pagination(self, mcp_client, project_id, work_item_with_history):
        ...
        assert result.data is not None, f"Should successfully retrieve paginated history but got: {result}"
        assert len(result.data) <= 2, f"Should return at most 2 revisions but got {len(result.data)}"

    async def test_get_work_item_history_with_expand(self, mcp_client, project_id, work_item_with_history):
        ...
        assert result.data is not None, f"Should successfully retrieve expanded history but got: {result}"
    ...
    async def test_get_work_item_history_invalid_work_item(self, mcp_client, project_id):
        ...
        with pytest.raises(Exception) as exc_info:
            await mcp_client.call_tool("get_work_item_history", {
                "project_id": project_id,
                "work_item_id": invalid_work_item_id
            })
    ...
    async def test_get_work_item_history_with_date_filter(self, mcp_client, project_id, work_item_with_history):
        ...
        assert result.data is not None, f"Should successfully retrieve filtered history but got: {result}"
        assert len(result.data) >= 0, f"Should return history data (may be empty) but got {len(result.data)} entries"
        ...
        assert result.data is not None, f"Should successfully retrieve filtered history but got: {result}"
        assert len(result.data) == 0, f"Should return no revisions for future date but got {len(result.data)} entries"

    async def test_get_work_item_history_with_date_range(self, mcp_client, project_id, work_item_with_history):
        ...
        assert result.data is not None, f"Should successfully retrieve history with date range but got: {result}"
        ...
        for revision in result.data:
            assert "revised_date" in revision, f"Revision should have revised_date field but got keys: {list(revision.keys())}"
...
class TestCommentsHistoryIntegration:
    ...
    async def test_comment_appears_in_history(self, mcp_client, project_id, test_work_item):
        ...
        initial_history = await mcp_client.call_tool("get_work_item_history", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        ...
        # Get updated history
        updated_history = await mcp_client.call_tool("get_work_item_history", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        
        assert len(updated_history.data) > initial_rev_count, f"History should have more revisions after adding comment: initial={initial_rev_count}, updated={len(updated_history.data)}"
    ...
    async def test_comment_and_update_workflow(self, mcp_client, project_id, test_work_item):
        ...
        comments = await mcp_client.call_tool("get_work_item_comments", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })
        
        history = await mcp_client.call_tool("get_work_item_history", {
            "project_id": project_id,
            "work_item_id": work_item_id
        })

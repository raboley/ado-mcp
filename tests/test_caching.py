...
class TestAdoCache:
    ...
    def test_cache_expiration(self):
        ...
        assert retrieved == test_data, (
            f"Cache entry should be available immediately. Expected {test_data} but got {retrieved}"
        )

        time.sleep(1.1)
        expired_result = self.cache._get("test")
        assert expired_result is None, (
            f"Cache entry should expire after TTL but got {expired_result}"
        )
    ...
    def test_project_caching(self):
        ...
        assert cached_projects[0].name == "ado-mcp", (
            f"Expected first project name 'ado-mcp' but got '{cached_projects[0].name}'"
        )
    ...
    def test_pipeline_caching(self):
        ...
        assert pipeline is not None, (
            "Should find pipeline by partial name fuzzy matching but got None"
        )
        assert pipeline.id == 1, f"Expected pipeline ID 1 but got {pipeline.id}"
    ...
    def test_cache_stats(self):
        ...
        assert stats["total_entries"] == 0, (
            f"Expected 0 total entries but got {stats['total_entries']}"
        )
        assert stats["active_entries"] == 0, (
            f"Expected 0 active entries but got {stats['active_entries']}"
        )
        ...
        assert stats["total_entries"] == 2, (
            f"Expected 2 total entries but got {stats['total_entries']}"
        )
        assert stats["active_entries"] == 2, (
            f"Expected 2 active entries but got {stats['active_entries']}"
        )

        time.sleep(1.1)
        stats = self.cache.get_stats()
        assert stats["total_entries"] == 2, (
            f"Expected 2 total entries after expiration but got {stats['total_entries']}"
        )
        assert stats["expired_entries"] == 1, (
            f"Expected 1 expired entry but got {stats['expired_entries']}"
        )
    ...
    def test_clear_expired(self):
        ...
        assert initial_count == 2, (
            f"Should have 2 entries before expiration but got {initial_count}"
        )
...
class TestCachingIntegration:
    def test_name_based_project_lookup(self):
        ...
        if projects:
            ...
            assert project.name == first_project_name, (
                f"Expected project name '{first_project_name}' but got '{project.name}'"
            )

            project2 = client.find_project_by_name(first_project_name)
            assert project2 is not None, (
                f"Cached lookup should find project '{first_project_name}' but got None"
            )
            assert project2.id == project.id, (
                f"Cached project should have same ID {project.id} but got {project2.id}"
            )
            ...
            for project_name in projects:
                ...
                if pipelines:
                    ...
                    assert project.name == project_name, (
                        f"Expected project name '{project_name}' but got '{project.name}'"
                    )
                    assert pipeline.name == pipeline_name, (
                        f"Expected pipeline name '{pipeline_name}' but got '{pipeline.name}'"
                    )
                    break
    ...
    def test_cache_performance(self):
        ...
        assert projects1 == projects2, (
            f"Cached and uncached results should be identical. First call returned {projects1} but cached call returned {projects2}"
        )

        assert second_call_time < first_call_time / 2, (
            f"Cached call ({second_call_time:.3f}s) should be faster than half of uncached call ({first_call_time:.3f}s) but was not"
        )
    ...
    def test_fuzzy_matching_with_real_data(self):
        ...
        if projects:
            ...
            if len(full_name) > 3:
                ...
                if project:
                    assert partial_name.lower() in project.name.lower(), (
                        f"Fuzzy match should contain partial name '{partial_name}' in found project '{project.name}' but does not"
                    )

...
class TestNameBasedPipelineOperations:
    ...
    async def test_run_pipeline_and_get_outcome_exact_name(self, mcp_client: Client):
        ...
        assert "execution_time_seconds" in result.data, "Result should contain execution_time_seconds"
    ...
    async def test_run_pipeline_and_get_outcome_case_insensitive(self, mcp_client: Client):
        ...
        assert result.data is not None, f"Should successfully run pipeline with case-insensitive name '{pipeline_name}'"
        assert "success" in result.data, "Result should contain success field"
    ...
    async def test_run_pipeline_and_get_outcome_fuzzy_match(self, mcp_client: Client):
        ...
        assert result.data is not None, f"Should successfully run pipeline with fuzzy matching for '{pipeline_name}'"
        assert "success" in result.data, "Result should contain success field"
    ...
    async def test_run_pipeline_with_variables(self, mcp_client: Client):
        ...
        assert result.data is not None, f"Should successfully start pipeline '{pipeline_name}' with variables"
        assert "id" in result.data, "Result should contain run ID"
    ...
    async def test_invalid_pipeline_name_error_with_suggestions(self, mcp_client: Client):
        ...
        with pytest.raises(Exception) as exc_info:
            await mcp_client.call_tool("run_pipeline_and_get_outcome", {
                "project_name": project_name,
        ...
        assert "not found" in error_message.lower(), f"Error should indicate pipeline not found, but got: {error_message}"
        assert ("did you mean" in error_message.lower() or 
               "available pipelines" in error_message.lower() or
               "similar pipelines" in error_message.lower()), f"Error should provide information about available pipelines, but got: {error_message}"
    ...
    async def test_invalid_project_name_error_with_suggestions(self, mcp_client: Client):
        ...
        with pytest.raises(Exception) as exc_info:
            await mcp_client.call_tool("run_pipeline", {
        ...
        assert "not found" in error_message.lower(), f"Error should indicate project not found, but got: {error_message}"
        assert ("did you mean" in error_message.lower() or 
               "available projects" in error_message.lower() or
               "similar projects" in error_message.lower()), f"Error should provide project information, but got: {error_message}"
    ...
    async def test_fuzzy_matching_performance(self, mcp_client: Client):
        ...
        assert elapsed_ms < 400, f"Fuzzy matching should complete quickly but took {elapsed_ms:.1f}ms"
    ...
    async def test_error_message_token_limiting(self, mcp_client: Client):
        ...
        try:
            ...
            assert estimated_tokens < 1000, f"Error message should be token-limited. Estimated tokens: {estimated_tokens:.0f}"
            ...
            assert ("did you mean" in error_message.lower() or 
                   "available pipelines" in error_message.lower() or
                   "similar pipelines" in error_message.lower()), "Error should provide suggestions"
    ...
    async def test_pipeline_name_variations(self, mcp_client: Client):
        ...
        for variation in name_variations:
            try:
                result = await mcp_client.call_tool("run_pipeline", {
                    "project_name": project_name,
                    "pipeline_name": variation
                })
                
                # Should succeed with fuzzy matching
                assert result.data is not None, f"Should match '{variation}' to 'test_run_and_get_pipeline_run_details'"
                assert "id" in result.data, f"Should return run ID for '{variation}'"
                
            except Exception as e:
                # If it fails, the error should still provide suggestions
                error_msg = str(e)
                assert "test_run_and_get_pipeline_run_details" in error_msg.lower(), f"Error for '{variation}' should suggest correct pipeline name"

...
async def test_analyze_pipeline_input_with_url(mcp_client: Client):
    ...
    result = await mcp_client.call_tool("analyze_pipeline_input", {
        "user_input": test_url
    })
    ...
    assert any("get_build_by_id" in step for step in analysis["next_steps"]), "Should suggest get_build_by_id"
    
    print(f"✓ Successfully analyzed URL: {analysis['input_type']} with {analysis['confidence']} confidence")
...
async def test_analyze_pipeline_input_with_pipeline_name(mcp_client: Client):
    ...
    result = await mcp_client.call_tool("analyze_pipeline_input", {
        "user_input": "log-test-complex pipeline failed"
    })
...
async def test_find_pipeline_by_name_fuzzy_match(mcp_client: Client):
    ...
    if search_result["matches"]:
        ...
        for match in search_result["matches"]:
            ...
            assert match["match_type"] in ["exact", "contains", "contained_in", "word_match"], "Should have valid match type"
...
async def test_resolve_pipeline_from_url_build_results(mcp_client: Client):
    ...
    result = await mcp_client.call_tool("resolve_pipeline_from_url", {
        "url": test_url
    })
    ...
    assert any("get_pipeline_run" in action for action in resolution["suggested_actions"]), "Should suggest get_pipeline_run"
...
async def test_helper_tools_tool_registration(mcp_client: Client):
    ...
    assert "Intelligently analyze user input" in analyze_tool.description, "Should have descriptive description"
    
    # Verify input schema
    input_schema = analyze_tool.inputSchema
    assert "user_input" in input_schema["properties"], "Should have user_input parameter"
    assert "organization" in input_schema["properties"], "Should have optional organization parameter"
...
async def test_helper_tools_no_client(mcp_client_with_unset_ado_env: Client):
    ...
    result = await mcp_client_with_unset_ado_env.call_tool("analyze_pipeline_input", {
        "user_input": "test"
    })
...
async def test_analyze_pipeline_input_with_yaml_file(mcp_client: Client):
    ...
    assert analysis["input_type"] in ["yaml_reference", "text_with_urls"], "Should identify YAML reference type"

...
from src.test_config import (
    ...
    get_parameterized_pipeline_id,
    get_preview_pipeline_id,
    get_github_resources_pipeline_id,
    get_runtime_variables_pipeline_id,
)
...
def validate_test_environment_setup(test_config):
    ...
    if not validation.get("config_loaded", False):
        pytest.skip(f"Test environment not configured: {validation.get('error')}. {validation.get('suggestion', '')}")
...
def pytest_configure():
    ...
    try:
        ...
        setattr(current_module, 'TEST_PROJECT_ID', config.project_id)
        setattr(current_module, 'PROJECT_NAME', config.project_name)

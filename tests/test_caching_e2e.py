...
class TestCachingE2E:
    ...
    def test_project_list_caching_behavior(self, telemetry_setup, fresh_cache):
        ...
        assert analyzer1.was_data_fetched_from_api("projects"), "First call should fetch from API, but data was retrieved from cache"
        assert analyzer1.count_api_calls() == 1, f"Expected 1 API call on first request, but got {analyzer1.count_api_calls()}"
        assert analyzer1.count_cache_misses() > 0, f"First call should have cache misses, but got {analyzer1.count_cache_misses()} misses"
        ...
        assert analyzer2.was_data_fetched_from_cache("projects"), "Second call should use cache, but data was fetched from API"
        assert analyzer2.count_api_calls() == 0, f"Expected 0 API calls on cached request, but got {analyzer2.count_api_calls()} calls"
        assert analyzer2.count_cache_hits() > 0, f"Second call should have cache hits, but got {analyzer2.count_cache_hits()} hits"
        
        assert projects1 == projects2, f"Projects from API and cache should be identical. API returned {projects1} but cache returned {projects2}"
    ...
    def test_pipeline_caching_with_name_lookup(self, telemetry_setup, fresh_cache):
        ...
        assert api_calls > 0, f"First pipeline lookup should make API calls, but got {api_calls} calls"
        ...
        api_calls_second = len([s for s in spans_after_second 
                               if "ado_" in s.name and "pipelines" in str(s.attributes)])
        assert api_calls_second == 0, f"Cached pipeline lookup should not make API calls, but made {api_calls_second} calls"
        
        assert pipelines1 == pipelines2, f"Pipelines from API and cache should be identical. API returned {pipelines1} but cache returned {pipelines2}"
    ...
    def test_cache_expiration_behavior(self, telemetry_setup, fresh_cache):
        ...
        try:
            ...
            assert analyzer_cached.was_data_fetched_from_cache("projects"), "Second call should use cache, but data was fetched from API"
            ...
            assert analyzer_expired.was_data_fetched_from_api("projects"), "Call after expiration should hit API, but data was retrieved from cache"
            
            assert projects1 == projects3, f"Projects should be consistent even after cache expiration. Initial call returned {projects1} but post-expiration call returned {projects3}"
    ...
    def test_fuzzy_matching_performance(self, telemetry_setup, fresh_cache):
        ...
        for test_name in test_names:
            if len(project_name) > 3 or test_name != project_name[:3]:
                ...
                assert api_calls == 0, f"Fuzzy match '{test_name}' should not make API calls but made {api_calls} calls"
    ...
    def test_name_based_pipeline_execution(self, telemetry_setup, fresh_cache):
        ...
        assert analyzer.count_cache_hits() > 0, f"Should have cache hits when finding cached pipeline, but got {analyzer.count_cache_hits()} hits"
        api_spans = [s for s in spans if s.name.startswith("ado_")]
        assert len(api_spans) == 0, f"Should not make API calls when data is cached, but made {len(api_spans)} API calls"
    ...
    def test_cache_statistics_accuracy(self, telemetry_setup, fresh_cache):
        ...
        assert stats["total_entries"] > 0, f"Cache should have entries but got {stats['total_entries']} entries"
        assert stats["active_entries"] > 0, f"Cache should have active entries but got {stats['active_entries']} active entries"
        
        spans = memory_exporter.get_finished_spans()
        analyzer = SpanAnalyzer(spans)
        
        assert analyzer.count_cache_hits() > 0, f"Should have cache hits recorded in telemetry but got {analyzer.count_cache_hits()} hits"
        assert analyzer.count_cache_misses() > 0, f"Should have cache misses for initial load but got {analyzer.count_cache_misses()} misses"
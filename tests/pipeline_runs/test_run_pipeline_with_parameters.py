...
async def test_run_pipeline_with_github_resources_stable_branch(mcp_client: Client):
    ...
    assert pipeline_run is not None, f"Expected pipeline run data but got None, full result: {result}"
    assert pipeline_run["id"] is not None, f"Expected pipeline run ID but got None, pipeline run: {pipeline_run}"
...
async def test_run_pipeline_parameter_combinations_tool_registration():
    async with Client(mcp) as client:
        ...
        assert run_pipeline_tool is not None, f"Expected run_pipeline tool to be registered but not found in tools: {[t.name for t in tools]}"
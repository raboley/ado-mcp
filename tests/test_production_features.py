...
class TestConfiguration:
    ...
    def test_config_from_environment(self):
        with patch.dict(os.environ, {
            ...
            assert config.organization_url == 'https://test.visualstudio.com', f"Expected organization_url 'https://test.visualstudio.com' but got '{config.organization_url}'"
            assert config.pat == 'test-pat', f"Expected pat 'test-pat' but got '{config.pat}'"
            assert config.retry.max_retries == 5, f"Expected max_retries 5 but got {config.retry.max_retries}"
            assert config.retry.initial_delay == 2.0, f"Expected initial_delay 2.0 but got {config.retry.initial_delay}"
            assert config.auth.timeout_seconds == 45, f"Expected auth timeout 45 but got {config.auth.timeout_seconds}"
            assert config.telemetry.enabled == False, f"Expected telemetry enabled False but got {config.telemetry.enabled}"
    
    def test_config_validation(self):
        with pytest.raises(AdoConfigurationError) as exc_info:
            RetryConfig(max_retries=-1)
        
        assert exc_info.value.error_code == "ADO_CONFIG_ERROR", f"Expected error_code 'ADO_CONFIG_ERROR' but got '{exc_info.value.error_code}'"
        assert "max_retries must be non-negative" in str(exc_info.value), f"Expected 'max_retries must be non-negative' in error message but got '{str(exc_info.value)}'"
    ...
    def test_config_overrides(self):
        ...
        assert config.organization_url == "https://override.visualstudio.com", f"Expected organization_url 'https://override.visualstudio.com' but got '{config.organization_url}'"
        assert config.request_timeout_seconds == 60, f"Expected request_timeout_seconds 60 but got {config.request_timeout_seconds}"
...
class TestAuthenticationChaining:
    ...
    def test_auth_manager_provider_chain(self):
        ...
        assert credential.token == "test-pat", f"Expected token 'test-pat' but got '{credential.token}'"
        assert credential.auth_type == "basic", f"Expected auth_type 'basic' but got '{credential.auth_type}'"
    ...
    def test_auth_manager_fallback(self):
        ...
        with patch.dict(os.environ, {'AZURE_DEVOPS_EXT_PAT': 'env-pat'}):
            ...
            credential = auth_manager.get_credential()
            assert credential.method == "env_pat", f"Expected method 'env_pat' but got '{credential.method}'"
            assert credential.token == "env-pat", f"Expected token 'env-pat' but got '{credential.token}'"
    ...
    def test_auth_manager_no_providers_succeed(self):
        ...
        assert exc_info.value.error_code == "ADO_AUTH_FAILED", f"Expected error_code 'ADO_AUTH_FAILED' but got '{exc_info.value.error_code}'"
        assert "No authentication method succeeded" in str(exc_info.value), f"Expected 'No authentication method succeeded' in error message but got '{str(exc_info.value)}'"
    ...
    def test_auth_manager_credential_caching(self):
        ...
        assert mock_provider.get_credential.call_count == 1, f"Expected provider called 1 time after first call but was called {mock_provider.get_credential.call_count} times"
        
        credential2 = auth_manager.get_credential()
        assert mock_provider.get_credential.call_count == 1, f"Expected provider called 1 time after cache hit but was called {mock_provider.get_credential.call_count} times"
        assert credential1.token == credential2.token, f"Expected cached credential token '{credential1.token}' but got '{credential2.token}'"
    ...
    def test_auth_manager_cache_invalidation(self):
        ...
        credential2 = auth_manager.get_credential()
        assert credential1.token == credential2.token, f"Expected same token '{credential1.token}' from fresh credential but got '{credential2.token}'"
...
class TestRetryMechanism:
    ...
    def test_retry_manager_eventual_success(self):
        ...
        assert result == "success", f"Expected result 'success' but got '{result}'"
        assert call_count == 3, f"Expected function called 3 times but was called {call_count} times"
    ...
    def test_retry_manager_max_retries_exceeded(self):
        ...
        assert "Network error" in str(exc_info.value), f"Expected 'Network error' in exception message but got '{str(exc_info.value)}'"
    ...
    def test_retry_manager_non_retryable_error(self):
        ...
        assert "Not found" in str(exc_info.value), f"Expected 'Not found' in exception message but got '{str(exc_info.value)}'"
...
class TestTelemetryIntegration:
    ...
    def test_telemetry_manager_initialization(self):
        ...
        assert telemetry.config.enabled == True, f"Expected telemetry enabled True but got {telemetry.config.enabled}"
        assert telemetry.config.service_name == "ado-mcp", f"Expected service_name 'ado-mcp' but got '{telemetry.config.service_name}'"
    
    def test_telemetry_disabled(self):
        config = TelemetryConfig(enabled=False)
        telemetry = TelemetryManager(config)
        
        assert telemetry.tracer is None, f"Expected tracer to be None when disabled but got {telemetry.tracer}"
        assert telemetry.meter is None, f"Expected meter to be None when disabled but got {telemetry.meter}"
    ...
    def test_telemetry_trace_context(self):
        ...
        assert context_works, f"Expected trace context manager to work without errors but got exception: {error_msg if not context_works else 'none'}"
    ...
    def test_telemetry_auth_recording(self):
        ...
        assert recording_works, f"Expected auth recording to work without errors but got exception: {error_msg if not recording_works else 'none'}"
...
class TestClientIntegration:
    ...
    def test_client_with_production_config(self):
        ...
        assert client.organization_url == "https://test.visualstudio.com", f"Expected organization_url 'https://test.visualstudio.com' but got '{client.organization_url}'"
        assert client.auth_method == "explicit_pat", f"Expected auth_method 'explicit_pat' but got '{client.auth_method}'"
        assert client.config.retry.max_retries == 5, f"Expected max_retries 5 but got {client.config.retry.max_retries}"
        assert client.config.auth.timeout_seconds == 45, f"Expected auth timeout 45 but got {client.config.auth.timeout_seconds}"
    ...
    def test_client_authentication_refresh(self):
        ...
        assert client.auth_method == "explicit_pat", f"Expected auth_method to remain 'explicit_pat' after refresh but got '{client.auth_method}'"
    ...
    def test_client_handles_rate_limiting(self):
        ...
        with patch('requests.request', return_value=mock_response):
            ...
            assert exc_info.value.error_code == "ADO_RATE_LIMIT", f"Expected error_code 'ADO_RATE_LIMIT' but got '{exc_info.value.error_code}'"
            assert exc_info.value.retry_after == 1, f"Expected retry_after 1 but got {exc_info.value.retry_after}"
    ...
    def test_client_handles_network_errors(self):
        ...
        with patch('requests.request', side_effect=RequestException("Network error")):
            ...
            assert exc_info.value.error_code == "ADO_NETWORK_ERROR", f"Expected error_code 'ADO_NETWORK_ERROR' but got '{exc_info.value.error_code}'"
            assert "Network error" in str(exc_info.value), f"Expected 'Network error' in exception message but got '{str(exc_info.value)}'"
    ...
    def test_client_handles_timeout_errors(self):
        ...
        with patch('requests.request', side_effect=Timeout("Request timeout")):
            ...
            assert exc_info.value.error_code == "ADO_TIMEOUT", f"Expected error_code 'ADO_TIMEOUT' but got '{exc_info.value.error_code}'"
            assert exc_info.value.timeout_seconds == config.request_timeout_seconds, f"Expected timeout_seconds {config.request_timeout_seconds} but got {exc_info.value.timeout_seconds}"
    ...
    def test_client_correlation_id_tracking(self):
        ...
        assert client.correlation_id is not None, f"Expected correlation_id to not be None but got {client.correlation_id}"
        assert len(client.correlation_id) > 0, f"Expected correlation_id to have length > 0 but got length {len(client.correlation_id)}"
        
        client2 = AdoClient(organization_url="https://test.visualstudio.com", pat="test-pat", config=config)
        assert client.correlation_id != client2.correlation_id, f"Expected different correlation_ids but both clients have '{client.correlation_id}'"
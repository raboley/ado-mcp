...
class ProcessesClient:
    ...
    def get_project_process_id(self, project_id: str) -> str:
        ...
        try:
            ...
            if not process_id:
                raise AdoError(f"Could not find process template ID for project '{project_id}'", "process_id_not_found")
            ...
            raise AdoError(f"Failed to get process ID for project '{project_id}': {e}", "get_process_id_failed") from e
    ...
    def get_project_process_info(self, project_id: str) -> ProjectProcessInfo:
        ...
        try:
            ...
            raise AdoError(f"Failed to get process info for project '{project_id}': {e}", "get_process_info_failed") from e
    ...
    def get_process_details(self, process_id: str) -> Process:
        ...
        try:
            ...
            if (is_not_found_error or (is_bad_request_error and not is_zero_uuid)):
                ...
                try:
                    ...
                    try:
                        ...
                        for project in projects[:10]:  # Limit to first 10 projects to avoid too many calls
                            try:
                                ...
                                if project_process_info.currentProcessTemplateId == process_id:
                                    if project_process_info.processTemplateName:
                                        ...
                                        logger.info(f"Found process name '{process_name}' from project '{project['name']}'")
                                        break
                        ...
                        logger.debug(f"Could not retrieve projects list for process name lookup: {projects_error}")
            ...
            raise AdoError(f"Failed to get process details for process '{process_id}': {e}", "get_process_details_failed") from e
    ...
    def get_default_team_id(self, project_id: str) -> str:
        ...
        try:
            ...
            if not default_team_id:
                raise AdoError(f"Could not determine default team for project '{project_id}'", "default_team_not_found")
            ...
            raise AdoError(f"Failed to get default team ID for project '{project_id}': {e}", "get_default_team_failed") from e
    ...
    def get_work_item_template(
        ...
        try:
            ...
            raise AdoError(f"Failed to get work item template '{template_id}': {e}", "get_template_failed") from e
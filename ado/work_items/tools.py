...
def register_work_item_tools(mcp_instance, client_container):
    ...
    def get_work_items_batch(
        ...
        try:
            ...
            batch_operation_context = {
                ...
                "work_item_ids_sample": work_item_ids[:5] if len(work_item_ids) <= 10 else work_item_ids[:3] + ["..."] + work_item_ids[-2:],
                "has_field_filter": fields is not None,
                "field_count": len(fields) if fields else 0,
                "fields_sample": fields[:5] if fields and len(fields) <= 10 else (fields[:3] + ["..."] + fields[-2:] if fields and len(fields) > 10 else fields),
        ...
        try:
            ...
            for i, update in enumerate(work_item_updates):
                ...
                for op in operations:
                    op_type = f"{op.get('op', 'unknown')}:{op.get('path', 'unknown').split('/')[-1]}"
                    operation_types[op_type] = operation_types.get(op_type, 0) + 1
                    
                # Log first few operations for detailed debugging
                if i < 3:
                    logger.info(f"Update {i+1} - WorkItem {work_item_id}: {len(operations)} operations")
            ...
            if success_rate < 90.0 and error_policy == "omit":
                logger.warning(f"Low success rate: {success_rate:.1f}% - many work items may have failed to update")
            ...
            logger.info(f"Successfully updated {result_count} out of {update_count} requested work items")
    ...
    def delete_work_items_batch(
        ...
        try:
            ...
            deletions_per_second = successful_deletions / total_duration if total_duration > 0 else 0
            ...
            # Performance warnings
            if success_rate < 90.0 and error_policy == "omit":
                logger.warning(f"Low success rate: {success_rate:.1f}% - many work items may have failed to delete")
            if total_duration > 10.0:  # 10 second threshold for batch deletions
                logger.warning(f"Slow batch deletion: {total_duration:.2f}s for {delete_count} items")
            
            logger.info(f"Successfully deleted {successful_deletions} out of {delete_count} requested work items")
